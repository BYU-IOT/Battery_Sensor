...
bool AutoConnect::begin(const char* ssid, const char* passphrase, unsigned long timeout) {
  ...
  //credential.load(0, &_credential); //.......................load server info from 1st position

  // Try to connect by STA immediately.
  // if (_credential.ssid == nullptr && _credential.password == nullptr)
  //   WiFi.begin();
  // else
  ...
  if (credential.entries() > 0) {
    ...
    if (nn > 0) {
      ...
      for (uint8_t i = 0; i < 1; i++) {
        ...
        AC_DBG("//..........................serverName %s\n", _credential.serverName);
        AC_DBG("//..........................sensorName %s\n", _credential.sensorName);
        AC_DBG("//..........................interval %s\n", _credential.interval);
        AC_DBG("//..........................adcFrq %s\n", _credential.adcFrq);
...
String AutoConnect::_induceConnect(PageArgument& args) {
  ...
  if (args.hasArg(AUTOCONNECT_PARAMID_CRED)) {
    ...
    strncpy(reinterpret_cast<char*>(_credential.serverName), reinterpret_cast<const char*>(entry.serverName), sizeof(_credential.serverName)); //........... store serverName in object
    strncpy(reinterpret_cast<char*>(_credential.sensorName), reinterpret_cast<const char*>(entry.sensorName), sizeof(_credential.sensorName)); //........... store sensorName in object
    strncpy(reinterpret_cast<char*>(_credential.interval), reinterpret_cast<const char*>(entry.interval), sizeof(_credential.interval)); //........... store interval in object
    strncpy(reinterpret_cast<char*>(_credential.adcFrq), reinterpret_cast<const char*>(entry.adcFrq), sizeof(_credential.adcFrq)); //........... store adcFrq in object
    ...
    strncpy(reinterpret_cast<char*>(_credential.serverName), args.arg(AUTOCONNECT_PARAMID_SERVER_NAME).c_str(), sizeof(_credential.serverName)); //........... store serverName in object
    strncpy(reinterpret_cast<char*>(_credential.sensorName), args.arg(AUTOCONNECT_PARAMID_SENSOR_NAME).c_str(), sizeof(_credential.sensorName)); //........... store sensorName in object
    strncpy(reinterpret_cast<char*>(_credential.interval), args.arg(AUTOCONNECT_PARAMID_INTERVAL).c_str(), sizeof(_credential.interval)); //........... store interval in object
    strncpy(reinterpret_cast<char*>(_credential.adcFrq), args.arg(AUTOCONNECT_PARAMID_ADCFRQ).c_str(), sizeof(_credential.adcFrq)); //........... store adcFrq in object
