...
int8_t AutoConnectCredential::load(const char* ssid, struct station_config* config) {
  ...
  if (_entries) {
    ...
    for (uint8_t i = 0; i < _entries; i++) {
      _retrieveEntry(reinterpret_cast<char*>(config->ssid), reinterpret_cast<char*>(config->password), reinterpret_cast<char*>(config->serverName), reinterpret_cast<char*>(config->sensorName), reinterpret_cast<char*>(config->interval), reinterpret_cast<char*>(config->adcFrq), config->bssid); //......... gets added values too
...
bool AutoConnectCredential::save(const struct station_config* config) {
  ...
  uint8_t eSize = strlen((const char*)config->ssid) + strlen((const char*)config->password) + strlen((const char*)config->serverName) + strlen((const char*)config->sensorName) + strlen((const char*)config->interval) + strlen((const char*)config->adcFrq) + sizeof(station_config::bssid) + 2; //..............Added the three new values
  ...
  dt = config->serverName;
  do {  //..........write serverName to memory
    c = *dt++;
    EEPROM.write(_dp++, c);
  } while (c != '\0');
  dt = config->sensorName;
  do {  //..........write sensorName to memory
    c = *dt++;
    EEPROM.write(_dp++, c);
  } while (c != '\0');
  dt = config->interval;
  do {  //..........write interval to memory
    c = *dt++;
    EEPROM.write(_dp++, c);
  } while (c != '\0');
  dt = config->adcFrq;
  do {  //..........write adcFrq to memory
    c = *dt++;
    EEPROM.write(_dp++, c);
  } while (c != '\0');
  for (uint8_t i = 0; i < sizeof(station_config::bssid); i++)
 ...
 */
void AutoConnectCredential::_retrieveEntry(char* ssid, char* password, char* serverName, char* sensorName, char* interval, char* adcFrq, uint8_t* bssid) { //................ added three new values to the input
  uint8_t ec;
